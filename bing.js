...
const outputpath = "bing.com";


//https://www.bing.com/search?q=bing+url+param+num+results&count=50
...
const customconfig = require(`${config.server.server.config}\\bing.js`);
...
async function dojob() {
    ...
    if (questions.length > 0) {
        for (var i = 0; i < questions.length; i++) {
            for (var j = 0; j < keywords.length; j++) {
                searchterms.push({
                    search: `intitle: "${questions[i]}"  ${keywords[j]}`,
                    key: keywords[i]
                });
            }
        }
    ...
    for (var i = 0; i < searchterms.length; i++) {
        urls.push({
            search: `https://www.bing.com/search?q=${encodeURIComponent(searchterms[i].search)}`,
    ...
    for (var i = 0; i < urls.length; i++) {
        try {
            ...
            await page.goto(`${urls[i].search}&count=50`, { waitUntil: 'networkidle2' });
            ...
            var noresults = $(`li.b_no`).length;
            if (!(noresults > 0)) {
                var number = parseInt($(`li.b_pag a[href][aria-label].sb_bp:last`).text());
    ...
    for (var i = 0, l = urlswithpages.length; i < l; i++) {
        ...
        for (var k = 0, l3 = urlswithpages[i].pages; k < l3; k++) {
            try {
                console.log(`${urlswithpages[i].url}&first=${((k * 50) - 1) > 0 ? ((k * 50) - 1) : 0}&count=50`);
                await client.send('Network.clearBrowserCookies');
                await client.send('Network.clearBrowserCache');
                await client.send('Network.setCacheDisabled', {
                    cacheDisabled: true,
                ...
                try {
                    l3 = parseInt($(`li.b_pag a[href][aria-label].sb_bp:last`).text());
                } catch (e) {}
                ...
                $(`li.b_algo`).each(function() {
                    try {
                        if ($(this).find(`h2`).first().text().trim().length > 10) {
                            ...
                            try {
                                o.TITLE = $(this).find(`h2`).first().text().trim();
                            ...
                            try {
                                o.TEXT = $(this).find(`p`).first().text().trim();
