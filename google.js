...
const customconfig = require(`${CONFIG}\\google.js`);
    ...
    for (var i = 0, l = urlswithpages.length; i < l; i++) {
        ...
        for (var k = 0, l3 = urlswithpages[i].pages; k < l3; k++) {
            try {
                ...
                await page.goto(`${urlswithpages[i].url}&start=${(k * 50)}&num=50`, { waitUntil: 'networkidle2' });
                await helper.sleep(5);
                await page.waitForSelector(`body`);
                ...
                $(`div.g`).each(function() {
                    try {
                        if ($(this).find('a h3').text().trim().length > 10) {
                            let o = {}
                            ...
                            try {
                                o.TITLE = $(this).find('a h3').text().trim();
                            ...
                            try {
                                o.TEXT = $(this).find(`div[style="-webkit-line-clamp:2"]`).text().trim();
                            ...
                            try {
                                o.HREF = $(this).find('a').attr('href').trim();
                            } catch (e) {}
